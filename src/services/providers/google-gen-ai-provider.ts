// src/services/providers/google-gen-ai-provider.ts
...
import { parseDataUrl } from "../../utils/image-utils";
import { InferenceProvider } from "./base-inference-provider";
...
export class GoogleGenAIProvider extends InferenceProvider {
  ...
  constructor(inferenceConfig: InferenceParams) {
    ...
    const baseUrl =
      ...
      `${env.CLOUDFLARE_AI_GATEWAY_BASE_URL}/${env.CLOUDFLARE_ACCOUNT_ID}/${env.CLOUDFLARE_AI_GATEWAY_ID}/${Provider.GOOGLE_AI_STUDIO}`;
  ...
  private transformMessages(messages: InferenceRequest["messages"]): Content[] {
    return messages
